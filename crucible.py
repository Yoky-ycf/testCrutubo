...
class Configuration(object):
    ...
    def fill_from_defaults(self):
        self.url, self.username = self.url or 'http://localhost:8060', self.username or 'yuchengfeng'
        return self
    ...
    def fill_from_args(self, args):
        ...
        self.branch = args.branch or self.branch
...
EXAMPLE USAGE:
 ...
 %(executable)s -r repository1 -b branches/feature1
                        create a review and anchor it to repository1 and branch 'branches/feature1', instead of trying to detect the repository and branch automatically\n
        ...
        self.parser.add_argument('-b', '--branch', type=str, help='the branch to anchor to, use with --repository option\n\n')
        ...
        self.parser.add_argument('--patch', action='store_const', const=True, help='print the patch, instead of creating a review')
    ...
    def parse_args(self):
        ...
        if args.noanchor and args.branch:
            Console.error('Please choose either --noanchor or --repository with --branch')
            print(self.help_blurb())
            sys.exit(1)

        if args.branch and not args.repository:
            Console.error('Please choose --branch together with --repository')
            print(self.help_blurb())
            sys.exit(1)
...
class CrucibleRest:
    ...
    def add_patch_data(self, patch, request_dict={}):
        ...
        if self.configuration.repository or self.configuration.branch:
            request_dict['anchor'] = {}
            ...
            if self.configuration.branch:
                request_dict['anchor']['anchorPath'] = self.configuration.branch
...
class HgSource(PatchSource):
    ...
    def get_patch(self):
        ...
        return check_output([self.configuration.executables[self.executable()], diff_command, '--config', 'diff.git=' + diff_git])
    ...
    def get_patch(self):
        ...
        for line in opened:
            ...
            if match:
                ...
                if file_type in ['binary', 'xbinary', 'ubinary', 'uxbinary', 'tempobj', 'ctempobj', 'xtempobj', 'symlink']:
